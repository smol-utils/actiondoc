...
- **push**: [Add model like runner](#add-model-like-runner), [Build docker images (Nightly CI)](#build-docker-images-nightly-ci), [Build docker images (Past CI)](#build-docker-images-past-ci), [Build docker images (scheduled)](#build-docker-images-scheduled), [Build documentation](#build-documentation), [Build pr ci-docker](#build-pr-ci-docker), [Check Tiny Models](#check-tiny-models), [CodeQL Security Analysis](#codeql-security-analysis), [Doctests](#doctests), [New model PR merged notification](#new-model-pr-merged-notification), [Nvidia CI](#nvidia-ci), [Nvidia CI - Flash Attn](#nvidia-ci---flash-attn), [Nvidia CI with nightly torch](#nvidia-ci-with-nightly-torch), [Release](#release), [Release - Conda](#release---conda), [Secret Leaks](#secret-leaks), [Self-hosted runner (AMD mi250 scheduled CI caller)](#self-hosted-runner-amd-mi250-scheduled-ci-caller), [Self-hosted runner (benchmark)](#self-hosted-runner-benchmark), [Self-hosted runner (nightly-past-ci-caller)](#self-hosted-runner-nightly-past-ci-caller), [Self-hosted runner scale set (AMD mi325 scheduled CI caller)](#self-hosted-runner-scale-set-amd-mi325-scheduled-ci-caller), [Self-hosted runner scale set (AMD mi355 scheduled CI caller)](#self-hosted-runner-scale-set-amd-mi355-scheduled-ci-caller), [Slow tests on important models (on Push - A10)](#slow-tests-on-important-models-on-push---a10), [Update Transformers metadata](#update-transformers-metadata)
- **schedule**: [Build docker images (scheduled)](#build-docker-images-scheduled), [Build pr ci-docker](#build-pr-ci-docker), [Check Tiny Models](#check-tiny-models), [Doctests](#doctests), [Extras Smoke Test](#extras-smoke-test), [Nvidia CI](#nvidia-ci), [Nvidia CI - Flash Attn](#nvidia-ci---flash-attn), [Self-hosted runner (AMD scheduled CI caller)](#self-hosted-runner-amd-scheduled-ci-caller), [Self-hosted runner (Intel Gaudi3 scheduled CI caller)](#self-hosted-runner-intel-gaudi3-scheduled-ci-caller), [Self-hosted runner (nightly-past-ci-caller)](#self-hosted-runner-nightly-past-ci-caller), [Stale Bot](#stale-bot)
- **workflow_dispatch**: [Benchmark v2 Framework](#benchmark-v2-framework), [Benchmark v2 Scheduled Runner - A10 Single-GPU](#benchmark-v2-scheduled-runner---a10-single-gpu), [Benchmark v2 Scheduled Runner - MI325 Single-GPU](#benchmark-v2-scheduled-runner---mi325-single-gpu), [Build docker images (scheduled)](#build-docker-images-scheduled), [Build documentation](#build-documentation), [Check Permissions Advisor](#check-permissions-advisor), [CodeQL Security Analysis](#codeql-security-analysis), [Nvidia CI](#nvidia-ci), [Nvidia CI - Flash Attn](#nvidia-ci---flash-attn), [Self-hosted runner (Intel Gaudi3 scheduled CI caller)](#self-hosted-runner-intel-gaudi3-scheduled-ci-caller), [SSH into our runners](#ssh-into-our-runners)
- **repository_dispatch**: [Build docker images (scheduled)](#build-docker-images-scheduled), [Build pr ci-docker](#build-pr-ci-docker), [Check Tiny Models](#check-tiny-models), [Doctests](#doctests), [Nvidia CI](#nvidia-ci), [Nvidia CI - Flash Attn](#nvidia-ci---flash-attn), [Nvidia CI with nightly torch](#nvidia-ci-with-nightly-torch), [Self-hosted runner (Intel Gaudi3 scheduled CI caller)](#self-hosted-runner-intel-gaudi3-scheduled-ci-caller)
- **workflow_run**: [Nvidia CI with nightly torch](#nvidia-ci-with-nightly-torch), [Self-hosted runner (AMD mi250 scheduled CI caller)](#self-hosted-runner-amd-mi250-scheduled-ci-caller), [Self-hosted runner scale set (AMD mi325 scheduled CI caller)](#self-hosted-runner-scale-set-amd-mi325-scheduled-ci-caller), [Self-hosted runner scale set (AMD mi355 scheduled CI caller)](#self-hosted-runner-scale-set-amd-mi355-scheduled-ci-caller), [Upload PR Documentation](#upload-pr-documentation)
- **issue_comment**: [PR - build doc via comment](#pr---build-doc-via-comment), [PR comment GitHub CI](#pr-comment-github-ci), [PR Repo. Consistency Bot](#pr-repo-consistency-bot), [TRL CI bot](#trl-ci-bot)
- **pull_request_target**: [Anti-Slop](#anti-slop), [Assign PR Reviewers](#assign-pr-reviewers), [CircleCI Failure Summary Comment](#circleci-failure-summary-comment), [PR slow CI - Suggestion](#pr-slow-ci---suggestion)
- **pull_request**: [Build PR Documentation](#build-pr-documentation), [PR CI](#pr-ci), [Self-hosted runner (benchmark)](#self-hosted-runner-benchmark)
- **merge_group**: [Build PR Documentation](#build-pr-documentation)
...
| `HF_HUB_READ_TOKEN` | [Benchmark v2 Framework](#benchmark-v2-framework), [Check Tiny Models](#check-tiny-models), [model jobs (model_jobs.yml)](#model-jobs), [model jobs (model_jobs_intel_gaudi.yml)](#model-jobs-1), [Nvidia CI (job definitions)](#nvidia-ci-job-definitions), [PR comment GitHub CI](#pr-comment-github-ci), [Process failed tests](#process-failed-tests), [Self-hosted runner (benchmark)](#self-hosted-runner-benchmark), [Self-hosted runner (scheduled-intel-gaudi)](#self-hosted-runner-scheduled-intel-gaudi), [SSH into our runners](#ssh-into-our-runners) |
